autocode.state['overview/icon'] = function() {
  ...
  if (autocode.project.icon) {
    $('#overview-icon-content img').removeClass('login-icon').attr('src', 'data:image/svg+xml;base64,' + btoa(autocode.project.icon));
  } else {
    $('#overview-icon-content img').addClass('login-icon');
  }
  
  var code_mirror = $('#overview-icon-content .CodeMirror');
  var value = autocode.project.icon || '';
