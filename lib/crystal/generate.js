...
(function() {
  var colors, crypto, crystal, cson, error, extend, findVersions, fs, generate, handlebars, inject, loadModules, loadOutputs, loaded_modules, merge, mkdirp, mustache, parse, path, processModules, readdir, season, semver, skeemas, sortObject, userHome, yaml;
  ...
  loadModules = function(modules, host) {
    var engine, export_name, export_path, exported, helper, j, len, model_ver, module_alias, module_config, module_name, module_path, module_version, module_version_query, module_versions, module_versions_path, processor, schema, spec, template, transformer;
  ...
  generate = function(project) {
    var export_name, exported, exports, imports, j, len, model_ver, module_alias, module_name, module_version, module_version_query, module_versions, module_versions_path;
    loaded_modules = {};
        ...
        if (module_version_query.match(/^(\.|\/)/)) {
          ...
          if (fs.existsSync(module_versions_path)) {
            ...
            for (j = 0, len = module_versions.length; j < len; j++) {
              model_ver = module_versions[j];
              model_ver = semver.clean(model_ver);
              if (model_ver && semver.satisfies(model_ver, project.modules[module_name]) && (!module_version || semver.gt(model_ver, module_version))) {
                module_version = model_ver;
              }
            }
          }
    ...
    if (project.outputs) {
      ...
      return loadOutputs(project.outputs, imports, project, this.force);
    }
  };
  ...
  inject = function(template, injectors, remove_injector) {
    ...
    return template.replace(/([  |\t]+)?>>>[a-z_]*<<<\n?/ig, function(injector) {
      ...
      if (injectors && injectors[injector]) {
        ...
        } else if ((injectors[injector].substr(0, 1) === '/' || injectors[injector].substr(0, 2) === './') && fs.existsSync(".crystal/" + injectors[injector])) {
  ...
  parse = function(spec, project, processors) {
    var i, j, k, len, len1, proc, processor, ref, s;
    for (i in spec) {
      s = spec[i];
