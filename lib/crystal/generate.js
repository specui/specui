...
(function() {
  var colors, crypto, crystal, cson, error, extend, findVersions, force, fs, generate, handlebars, inject, loadModules, loadOutputs, loaded_modules, merge, mkdirp, mustache, parse, path, processModules, readdir, season, semver, skeemas, sortObject, userHome, yaml;
  ...
  loadOutputs = function(outputs, imports, config) {
    ...
    for (output_i in outputs) {
      ...
      if (generator.copy) {
        ...
        for (m = 0, len3 = code_files.length; m < len3; m++) {
          code_file = code_files[m];
